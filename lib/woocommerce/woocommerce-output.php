 ...
 * Wpts.
 *
 * This file adds the WooCommerce styles and the Customizer additions for WooCommerce to the Wpts Theme.
 *
 * @package Wpts
...
add_filter( 'woocommerce_enqueue_styles', 'wpts_woocommerce_styles' );
/**
 ...
 */
function wpts_woocommerce_styles( $enqueue_styles ) {

	$enqueue_styles[ genesis_get_theme_handle() . '-woocommerce-styles' ] = [
		'src'     => get_stylesheet_directory_uri() . '/lib/woocommerce/wpts-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', 'wpts_woocommerce_css' );
/**
...
function wpts_woocommerce_css() {
	...
	$color_link   = get_theme_mod( 'wpts_link_color', $appearance['default-colors']['link'] );
	$color_accent = get_theme_mod( 'wpts_accent_color', $appearance['default-colors']['accent'] );
	...
	',
		$color_accent,
		wpts_color_contrast( $color_accent )
	) : '';
