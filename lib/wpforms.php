<?php
/**
 * WebPagesThatSell WPForms helper functions.
 *
...
/**
 * Creates a WPForms form if one added by a WebPagesThatSell theme does not exist.
 *
 * @since 2.10.0
 *
 * @return int|null ID of form if one exists or gets created. Null if form creation fails or WPForms is inactive.
 */
function webpagesthatsell_maybe_create_wpforms_form() { // phpcs:ignore -- webpagesthatsell prefix for functions shared between themes.
	...
	$new_form_id = wpforms()->form->add(
		esc_html__( 'Simple Contact Form', 'wpts' ),
		[],
		[
 ...
 */
function webpagesthatsell_insert_contact_form( $content, $imported_posts ) { // phpcs:ignore -- webpagesthatsell prefix for functions shared between themes.

	$form_id = webpagesthatsell_maybe_create_wpforms_form();
